...
/*   Updated: 2019/02/15 15:17:47 by hehlinge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_atoi_base(char *str, char *base, int param)
{
	int nb;
	int charct;
	int is_negative;
	...
	charct = -1;
	is_negative = 0;
	if (param == 0 && base_is_valid(base) != -1 && str_is_valid(str, base, &is_negative, &charct) == 1)
	{
		while (ft_atoi_base(str + (++charct), base, 1) - 1 != -1 && str[charct] != '\0')
			nb = nb * base_is_valid(base) + ft_atoi_base(str + (++charct), base, 1) - 1;
		if (is_negative == 1)
			nb = -nb;
		return (nb);
	}
